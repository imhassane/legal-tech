          ...
          await this.$apollo.mutate({
            ...
            update: function(store, _) {
              try {
                ...
                let approvedArticles = store.readQuery({ query: APPROVED_QUERY });

                article = data.articles.filter(art => art.id === id);
                data.articles = data.articles.filter(art => art.id !== id );

                if(article)
                  approvedArticles.articles.push(article[0]);

                store.writeQuery({query: ARTICLES_QUERY, data})
                store.writeQuery({query: APPROVED_QUERY, data: approvedArticles});
              } catch(_) {}
            }
          });
        } catch(ex) {
