...
const members = async (_, {type, status, start, limit}, {pool}) => {
    try {
        let rows = [];
        if(type === "ALL")
            rows = (await pool.query(sql.GET_ALL_MEMBERS, [status, start, limit])).rows;
        else
            rows = (await pool.query(sql.GET_MEMBERS, [type, status, start, limit])).rows;
